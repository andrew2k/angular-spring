...
let HmacHttpClient = class HmacHttpClient extends http_1.Http {
    ...
    addSecurityHeader(url, method, options, body) {
        if (AppUtils.UrlMatcher.matches(url)) {
            let securityToken = new securityToken_1.SecurityToken(JSON.parse(localStorage.getItem(AppUtils.STORAGE_SECURITY_TOKEN)));
            let date = new Date().toISOString();
            let secret = securityToken.publicSecret;
            let message = '';
    ...
    post(url, body, options) {
        ...
        console.log('SONO NELLA CUSTOM POST');
        return Observable_1.Observable.create((observer) => {
