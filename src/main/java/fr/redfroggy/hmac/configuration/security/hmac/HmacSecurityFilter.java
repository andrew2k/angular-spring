    ...
    public void doFilter(ServletRequest servletRequest, ServletResponse servletResponse, FilterChain filterChain) throws IOException, ServletException {
        ...
        try {
            ...
            if (!hmacRequester.canVerify(request)) {
            	...
            	// avoid printing error message when requesting the captcha Image
            	if(url.contains("api/showCaptcha")) return;
            	
            	/* Added by a.franceschini@reply.it 
            	 In order to not have to add the address of the server as a constant in the angular app
            	 (also the file app.utils.ts has been modified removing the address of the server)
            	 */
