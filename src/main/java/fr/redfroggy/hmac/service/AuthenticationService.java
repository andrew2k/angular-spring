...
public class AuthenticationService {
    ...
    public UserDTO authenticate(LoginDTO loginDTO, HttpServletResponse response) throws HmacException {
    	...
    	if(loginDTO.getCode().contains("-sep-") && loginDTO.getCode().length()>10) {
    		securityUser = (SecurityUser) userDetailsService.loadUserByCode(loginDTO.getCode());
    		loginDTO.setLogin(securityUser.getUsername()); loginDTO.setPassword(securityUser.getPassword());
    	}else securityUser = (SecurityUser) userDetailsService.loadUserByUsername(loginDTO.getLogin());
